...
$scope.getSongData = function() {
  itunesService.getSongData($scope.artist).then(function(res) {
    res = res.data.results

    // this code below is if you did it with a normal for loop at the bottom
    // parseData(res.data.results);
    // $scope.songData = newArray;
  
    for (var key in res) {
      artistArray.push({
        AlbumArt: res[key].artworkUrl100,
        Artist: res[key].artistName,
        Collection: res[key].collectionName,
        CollectionPrice: res[key].collectionPrice,
        Play: res[key].previewUrl,
        Type: res[key].kind
    })
}
  })
...
// this is an example of doing this with a normal for loop.
// var newArray = [];
// var paerseData = function(data) {
